...
DOMAIN_GENERICS = {"design","designer","designers","trend","trends",
...
def build_vec(texts, min_df=3, max_df=0.7, ngram=(1,3), extra_stop=None):
    stop = set(ENGLISH_STOP_WORDS) | set(NOISE_STOPWORDS)
...
def main():
    ...
    ap.add_argument("--topic", required=True, choices=["decor","fashion","design","beauty"])
    ap.add_argument("--raw", required=True, help="data/<topic>_all_raw.csv")
    ap.add_argument("-o","--out-prefix", required=True, help="e.g., data/decor")
    ap.add_argument("--new_weeks", type=int, default=6)
    ap.add_argument("--min_df", type=int, default=3)
    ap.add_argument("--max_df", type=float, default=0.7)
    args = ap.parse_args()

    df = pd.read_csv(args.raw)
    if df.empty:
        for name in ["phrase_counts_over_time","top_phrases","increasing_terms","decreasing_terms","new_terms"]:
            pd.DataFrame().to_csv(f"{args.out_prefix}_{name}.csv", index=False)
        print(f"[{args.topic}] no data; wrote empty outputs"); return
    ...
    # hide ultra-generic unigrams (plus the topic word itself)
    generic_unigrams = set(DOMAIN_GENERICS) | {args.topic}
    ...
    print(f"[{args.topic}] topic analysis written under {args.out_prefix}_*.csv")
